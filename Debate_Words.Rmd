...
candidate_text_tc <- function(candidate, mydata){
    ...
    t_c <- tm_map(t_c, removeNumbers)
    t_c <- tm_map(t_c, removeWords, stopwords("english"))
    t_c <- tm_map(t_c, removeWords, c("applause", "thats", "laughter", "dont", "back", "can", "get", "cant", "come", "big"))
...
c_wordcloud <- function(t_c){
    ...
    wordcloud(t_c, scale=c(4,0.4), max.words=40, min.freq=5, random.order=FALSE, rot.per=0.2, use.r.layout=FALSE, colors=color_map2)
...
####THE ALSO-RANS: CRUZ AND HUCKABEE
...
More humorously, note that the word "clinton" figures in both Carly Fiorina's and Hilary Clinton's, whose texts almost read as an assertion by Hilary and an counter argument by Carly.  
Ted Cruz's frequent words again seem to focus on business and Carlo Rubio loves the homeland.  
  
So, while word frequencies reveal differences between the approaches and personalities of the candidates, they don't by themsleves elucidate differences on important policies. 
...
a<- bigram_finder("wall", trump_bigrams)[1:3]
